...
# Paired reads
    ...
    reads1_dict |= reads2_rev
    fastq_dict = reads1_dict
...
for i in range(len(ref_seq)):
    if i in read_index:
        ...
        if not sequence:
            sequence += 'N'
        elif sequence.count('N') == len(sequence):
            sequence += 'N'
        elif position >= readlen - 1:
            sequence += 'N'
        else:
            position += 1
...
if args.identity:
    per_i = identity(sequence,ref_seq,0,len(sequence))
    per_map = map_reads/len(fastq_dict)
    print('Percentage identity: {0:.2f}%'.format(per_i))
    print('Mapped reads: {0:.2f}%'.format(map_reads))